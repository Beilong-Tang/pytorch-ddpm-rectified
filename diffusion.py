...
class MatchedGaussianDiffusionTrainer(nn.Module):
...
class GaussianDiffusionSampler(nn.Module):
    ...
    def forward(self, x_T, add_noise = True):
        """
        Algorithm 2.
        # If add_noise is False , then noise is zero, it is simply just ODE
        """
        x_t = x_T
...
class DDIMSampler(nn.Module):
    ...
    def forward(self, x_t, steps: int = 1, method="linear", eta=0.0,
        ...
        x = [x_t]
        with tqdm(reversed(range(0, steps)), colour="#6565b5", total=steps, disable=True) as sampling_steps:
